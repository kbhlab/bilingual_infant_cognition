---
title: "Analyses Script - Dal Ben et al. (2021)"
...
It analyses two new datasets (Studies 1 & 3) and reanalyzes three open datasets (Studies 2a, 2b, 2c).
...
Please indicate which study (1, 2a, 2b, 2c, or 3) and which sample (filtered or unfiltered) you want to analyze. For Study 3 - filtered sample, it is also possible to include `vocabulary` as a predictor. 
...
library(flextable)
library(sjPlot)
library(htmltools) ## test multiple outputs
...
blues_palette <- colorRampPalette(c("#C6DBEF", "#082c58"))
...
if ((study == "1" | study == "2a" | study == "2b" | study == "2c" | study == "3") & (filtered == TRUE | filtered == FALSE)){
  
  if (filtered == TRUE & study != "3"){dataset = paste(study, "filtered", sep = "_")
  } else if (filtered == TRUE & study == "3"){
  
    if (vocabulary == TRUE){dataset = paste(study, "filtered", "vocabulary", sep = "_") 
    } else {dataset = paste(study, "filtered", sep = "_")}
    
  } else if (filtered == FALSE & study == "3" & vocabulary == TRUE){dataset = "ERROR"
...
if (study == "1" | study == "3"){
  ...
  load(here("output","data","exclusions_1_3.Rda"))
  exclusions_dataset <- 
    exclusions_1_3 %>% 
  ...
  if(filtered == TRUE){
    ...
    load(here("output","data","data_full_trials_filtered_1_3.rda")) 
    analysis_dataset = 
      data_full_trials_filtered_1_3 %>% 
      mutate(id = recording_name)
    
    load(here("output","data","descriptive_filtered_1_3.Rda"))
    
    
    descriptive_dataset <- descriptive_filtered_1_3 %>% 
    ...
    if (vocabulary == TRUE){
      ...
      analysis_dataset <- 
        descriptive_dataset %>%
        select(id) %>%
        inner_join(analysis_dataset, by = "id")
    ...
    load(here("output","data","data_full_trials_unfiltered_1_3.rda")) 
    analysis_dataset = 
      data_full_trials_unfiltered_1_3 %>% 
      mutate(id = recording_name)
    
    load(here("output","data","descriptive_unfiltered_1_3.Rda"))
    #fix one date of birth that is wrong in MSL
    
    descriptive_dataset <- descriptive_unfiltered_1_3 %>%
      rename(id = recording_name)
  ...
  if(filtered == TRUE){
    ...
    load(here("output","data","data_full_trials_filtered_2a.rda")) 
    data_full_trials_filtered_2a <- data_full_trials_filtered_1b
    rm(data_full_trials_filtered_1b)
    analysis_dataset <- data_full_trials_filtered_2a
    descriptive_dataset <- data_full_trials_filtered_2a
    
  } else if (filtered == FALSE){
    
    load(here("output","data","data_full_trials_unfiltered_2a.rda")) 
    data_full_trials_unfiltered_2a <- data_full_trials_unfiltered_1b
    rm(data_full_trials_unfiltered_1b)
    analysis_dataset <- data_full_trials_unfiltered_2a
    descriptive_dataset <- data_full_trials_unfiltered_2a
  ...
  if(filtered == TRUE){
    ...
    load(here("output","data","data_full_trials_filtered_2b.rda")) 
    analysis_dataset <- data_full_trials_filtered_2b
    descriptive_dataset <- data_full_trials_filtered_2b

  } else if (filtered == FALSE){
    
    load(here("output","data","data_full_trials_unfiltered_2b.rda")) 
    analysis_dataset <- data_full_trials_unfiltered_2b
    descriptive_dataset <- data_full_trials_unfiltered_2b
  ...
  if(filtered == TRUE){
    ...
    load(here("output","data","data_full_trials_filtered_2c.rda")) 
    analysis_dataset <- data_full_trials_filtered_2c
    descriptive_dataset <- data_full_trials_filtered_2c
    
  } else if (filtered == FALSE){
    
    load(here("output","data","data_full_trials_unfiltered_2c.rda")) 
    analysis_dataset <- data_full_trials_unfiltered_2c 
    descriptive_dataset <- data_full_trials_unfiltered_2c 
...
Note. Socio-economic-status was calculated differently for each study. Study 1 and 2 measured the mother educational level. Study 2a used "a composite of four weighted scores based on the carers’ (1) postcode (as an index of socioeconomic deprivation), (2) education attainment, (3) household income and (4) occupation." (D'Souza et al., 2020, p. 4). Study 2b and 2c measured both educational and income level. Here we calculate the descriptive statistics on educational level.

The following are available for some studies:
- sample count for age group (1 and 2);
- exclusion reason (1 and 2 filtered);
...
} else if (study == "2a" | study == "2b" | study == "2c"){
...
if (study == "3" & filtered == TRUE & vocabulary == TRUE){
...
if ((study == "1" | study == "3") & filtered == TRUE){
...
Language tables are available for Studies 1 and 3, for the unfiltered sample. 

For study 2a, D'Souza et al. (2020) article and supplementals do not provide information about participants' dominant/other languages or the percentages of exposure to different languages. Unable to produce language tables.

```{r tables}
##---------------------------------------------------------------- LANGUAGE TABLES------------------------------ 
if ((study == "1" | study == "3") & filtered == FALSE){

  load(here("output","data","descriptive_filtered_1_3.Rda")) 
  descriptive_filtered_1_3 <- descriptive_filtered_1_3 %>%
    rename(id = recording_name)#load filtered descriptives for comparing
  ...
  lt %>%
  select(id, dom_lang, per_eng, per_fr, other_lang1, per_other1) %>%
  ...
  pivot_wider(names_from = lang_order, values_from = lang_percent, id_cols = "id") %>%
  separate(L1, into = c("L1", "L1_pct"), sep = "_") %>%
  separate(L2, into = c("L2", "L2_pct"), sep = "_") %>%
  separate(L3, into = c("L3", "L3_pct"), sep = "_") %>%
  select(id, L1, L1_pct, L2, L2_pct, L3, L3_pct) %>%
  ...
  mutate(id2 = row_number(),
         ...
         `% 3` = case_when(`% 3` == "0" ~ "",
                           TRUE ~ `% 3`),
         `Filtered Sample` = case_when(id %in% descriptive_filtered_1_3$id ~ "yes",
                                       TRUE ~ "no")) %>%
  select(-id) %>%
  select(`participant` = id2, 1:6, `Filtered Sample`) %>%
  flextable() %>%
  ...
  italic(i = 1, part = "header")  %>% 
    save_as_docx(path = here("output","table_figure","table_lang_", dataset, ".docx"))
  
  rm(descriptive_unfiltered_1_3, lt)
  ...
  italic(i = 1, part = "header") %>% 
    save_as_docx(path =
                   here("output","table_figure","table_lang_",
                               dataset, ".docx"))
...
if (study == "1" | study == "3"){
  ...
  } else if (study == "3" & filtered == TRUE & vocabulary == TRUE){
...
data_eyetracking_full_trials <- analysis_dataset %>%
    {if (study == "1" | study == "3") left_join(., edu_years, by = "mother_edu_raw") else . } %>%
...
###----------------------------------------------------SET PARAMETERS
if (study == "1"){
  data_time_series_full <- data_time_series_full %>%  filter(age_group == "7 months")
  
  study_age <- "age 7 months"
}

if (study == "3"){
  data_time_series_full <- data_time_series_full %>%  filter(age_group == "20 months")
  
  study_age <- "age 20 months"
}

if (study == "2a"){
  
  study_age <- "age 7–9 months"
}

if (study == "2b") {
  
  study_age <- "age 7 months; Visual Condition"
}

if (study == "2c") {
  
  study_age <- "age 7 months; Auditory Condition"
}

if (study == "3" & vocabulary == TRUE){
  
  FIG_TITLE <- paste0("Time course of infant looking for Study ", study, " (", study_age, " - vocabulary )")
  
} else {
  
  FIG_TITLE <- paste0("Time course of infant looking for Study ", study, " (", study_age, ")")
...
time_series_full_trial <-
  ...
  filter(Time >= X_START & Time <= X_END) %>% # proportional to study 1
  ...
  geom_text(data = facet_text, label = facet_text$label, show.legend = FALSE, color = "#666666",
            size = 3) +
  facet_grid(fct_reorder(trial_labels, parse_number(as.character(trial_labels))) ~ factor(trial_type_labs, levels = c("Training", "Test"))) +
  ggtitle(FIG_TITLE) + 
...
#2700 x 2700 pixels for studies 1, 2a and 3
if (study == "1" | study == "2a" | study == "3") {
...
For study 2b and 2c, the `gmler()` show a convergence warning. after further inspection/testing with transformed variables (trial number), the model converged and the estimates were proportional. Thus, it is a false negative. See code below for further checking.
###Training phase
```{r}
###------------------------------------------------------------- TRAINING--------------
...
if (study == "3" & vocabulary == TRUE){
  ...
  #This model doesn't converge, but it's likely due to more trial numbers since it does converge, with similar estimates, when trial number is rescaled (see second model below)
    glmermodel_pre <- 
    model_data_pre %>% 
    mutate(trial_num_scaled = trial_num_scaled) %>%
    glmer(SamplesInAOI / SamplesTotal ~ 
            language * time_bin_centred * trial_num_scaled * language + 
            (1 | id), 
          data = ., 
          family = binomial, 
          weights = SamplesTotal, 
  ...
  glmermodel_pre_rescaled <- 
    model_data_pre %>% 
    mutate(trial_num_scaled = trial_num_scaled/2) %>%
    glmer(SamplesInAOI / SamplesTotal ~ 
            language * time_bin_centred * trial_num_scaled * language + 
            (1 | id), 
          data = ., 
...
tab_model(final_model_pre, 
          ...
          show.icc = FALSE, 
          title = TAB_TITLE,
          file = here("output","table_figure", paste0("glermodel_training_", 
                             dataset, ".doc")))


###----------------------------------------------------CONVERGENCE TESTS FOR 2b AND 2c-----------------
...
tab_model(final_model_post, 
          ...
          show.icc = FALSE, 
          title = TAB_TITLE, 
          file = here("output","table_figure", paste0("glermodel_test_", dataset,
                             ".doc")))
            
###----------------------------------------------------CONVERGENCE TESTS FOR 2b AND 2c-----------------
#the tt and ll code below is nowhere close to 0. So I think the regular model should be fine to report.

# lme4::allFit(show.meth.tab = TRUE)
...
} else if(study == "1" | study == "2a" | (study == "3" & vocabulary == FALSE)) {
...
if ((study == "1" | study == "3") & vocabulary == FALSE){
...
} else if (study == "3" & vocabulary == TRUE){
...
trial_data <- data_eyetracking_clean %>%
  make_time_window_data(aois = c("target", "distractor", "circle"),
...
plot_by_trial <- trial_data %>%
  filter(age_group == "7 months") %>%
  mutate(trial_type = case_when(trial_type == "pre-switch" ~ "Training",
                                TRUE ~ "Test"),
         trial_type = factor(trial_type, levels = c("Training", "Test"))) %>%
  group_by(trial_type, language, trial_num, age_group) %>%
  summarize(correct = mean(correct)) %>%
  ggplot(aes(x = trial_num, y = correct, color = language, shape = language, linetype = language)) +
  geom_point() +
  ...
  ggtitle("Correct anticipation by trial for Study 1 (age 7 months)")

plot_by_trial

ggsave(here("output","table_figure",paste0("plot_by_trial_", study, "_", filtered_name, ".png")), width = 9, height = 4)
...
plot_by_trial <- trial_data %>%
  ...
  ylab("Proportion of infants with \ncorrect anticipation") +
  labs(color  = "Language group", linetype = "Language group", shape = "Language group") +
  ggtitle("Correct anticipation by trial for Study 3 (age 20 months)")
...
apa.ezANOVA.table(pre_7,
                  table.number = 3,
                  correction = "none",
                  table.title = "Mixed ANOVA results for the Training phase in Study 1 (7-month-olds)",
                  filename = here("output","table_figure", paste0("anova_table_training_", dataset, ".doc")))
...
t_table_pre_7_mono_blocks <- broom::tidy(pre_7_mono_t_1_2) %>%
  bind_rows(broom::tidy(pre_7_mono_t_1_3)) %>%
  bind_rows(broom::tidy(pre_7_mono_t_2_3)) %>%
  rename(mean_diff = estimate,
         t = statistic,
         df = parameter) %>%
  mutate(cohen_d = t/sqrt(df +1))
...
t_table_pre_7_bi_blocks <- broom::tidy(pre_7_bi_t_1_2) %>%
  bind_rows(broom::tidy(pre_7_bi_t_1_3)) %>%
  bind_rows(broom::tidy(pre_7_bi_t_2_3)) %>%
  rename(mean_diff = estimate,
         t = statistic,
         df = parameter) %>%
  mutate(cohen_d = t/sqrt(df +1))
...
t_table_pre_7 <- broom::tidy(pre_7_t_1) %>%
  bind_rows(broom::tidy(pre_7_t_2)) %>%
  bind_rows(broom::tidy(pre_7_t_3)) %>%
...
apa.ezANOVA.table(post_7,
                  table.number = 3,
                  correction = "none",
                  table.title = "Mixed ANOVA results for the Test phase in Study 1 (7-month-olds)",
                  filename = here("output","table_figure", paste0("anova_table_test_", dataset, ".doc")))
...
#-------------------------------------------------------------------- MEANS
block_data %>%
  filter(age_group == "7 months" & trial_type == "pre-switch" & n_blocks == 3) %>%
  apa.2way.table(iv1 = block_num, 
               iv2 = language,
               dv = correct_anticipation, 
               data = ., 
               show.marginal.means = FALSE,
               filename = here("output","table_figure", paste0("means_table_training_", dataset, ".doc")),
               table.number = 4)
...
block_data %>%
  ...
  apa.2way.table(iv1 = block_num, 
               ...
               data = ., 
               show.marginal.means = FALSE,
               filename = here("output","table_figure", paste0("means_table_test_", dataset, ".doc")),
               table.number = 4)
...
t_table_post_7_mono_blocks <- broom::tidy(post_7_mono_t_1_2) %>%
  bind_rows(broom::tidy(post_7_mono_t_1_3)) %>%
  bind_rows(broom::tidy(post_7_mono_t_2_3)) %>%
  rename(mean_diff = estimate,
         t = statistic,
         df = parameter) %>%
  mutate(cohen_d = t/sqrt(df +1))
...
t_table_post_7_bi_blocks <- broom::tidy(post_7_bi_t_1_2) %>%
  bind_rows(broom::tidy(post_7_bi_t_1_3)) %>%
  bind_rows(broom::tidy(post_7_bi_t_2_3)) %>%
  rename(mean_diff = estimate,
         t = statistic,
         df = parameter) %>%
  mutate(cohen_d = t/sqrt(df +1))
...
t_table_post_7 <- broom::tidy(post_7_t_1) %>%
...
t_table_pre_mono_20_blocks <- broom::tidy(pre_20_mono_t_1_2) %>%
  bind_rows(broom::tidy(pre_20_mono_t_1_3)) %>%
  bind_rows(broom::tidy(pre_20_mono_t_2_3)) %>%
  rename(mean_diff = estimate,
         t = statistic,
         df = parameter) %>%
  mutate(cohen_d = t/sqrt(df +1))
...
t_table_pre_20_bi_blocks <- broom::tidy(pre_20_bi_t_1_2) %>%
  bind_rows(broom::tidy(pre_20_bi_t_1_3)) %>%
  bind_rows(broom::tidy(pre_20_bi_t_2_3)) %>%
  rename(mean_diff = estimate,
         t = statistic,
         df = parameter) %>%
  mutate(cohen_d = t/sqrt(df +1))
...
t_table_post_mono_20_blocks <- broom::tidy(post_20_mono_t_1_2) %>%
  bind_rows(broom::tidy(post_20_mono_t_1_3)) %>%
  bind_rows(broom::tidy(post_20_mono_t_2_3)) %>%
  rename(mean_diff = estimate,
         t = statistic,
         df = parameter) %>%
  mutate(cohen_d = t/sqrt(df +1))
...
t_table_post_20_bi_blocks <- broom::tidy(post_20_bi_t_1_2) %>%
  bind_rows(broom::tidy(post_20_bi_t_1_3)) %>%
  bind_rows(broom::tidy(post_20_bi_t_2_3)) %>%
  rename(mean_diff = estimate,
         t = statistic,
         df = parameter) %>%
  mutate(cohen_d = t/sqrt(df +1))
...
t_table_pre_20 <- broom::tidy(pre_20_t_1) %>%
  bind_rows(broom::tidy(pre_20_t_2)) %>%
  bind_rows(broom::tidy(pre_20_t_3)) 
...
post_20_t_1 <- block_data %>%
  filter(age_group == "20 months") %>%
  filter(trial_type == "post-switch") %>%
  filter(block_num == 1) %>%
  t.test(correct_anticipation ~ language, data = .)

post_20_t_2 <- block_data %>%
  filter(age_group == "20 months") %>%
  filter(trial_type == "post-switch") %>%
  filter(block_num == 2) %>%
  t.test(correct_anticipation ~ language, data = .)

post_20_t_3 <- block_data %>%
  filter(age_group == "20 months") %>%
  filter(trial_type == "post-switch") %>%
  filter(block_num == 3) %>%
  t.test(correct_anticipation ~ language, data = .)

t_table_post_20 <- broom::tidy(post_20_t_1) %>%
  bind_rows(broom::tidy(post_20_t_2)) %>%
  bind_rows(broom::tidy(post_20_t_3)) %>%
  rename(M_mono = estimate1,
         M_bi = estimate2,
         t = statistic,
         df = parameter) %>%
...
apa.ezANOVA.table(post_20,
                  table.number = 7,
                  correction = "none",
                  table.title = "Mixed ANOVA results for the Test phase in Study 3 (20-month-olds)",
                  filename =  here("output","table_figure", paste0("anova_table_test_", dataset, ".doc")))
...
#-------------------------------------------------------------------- MEANS
block_data %>%
  filter(age_group == "20 months" & trial_type == "pre-switch" & n_blocks == 3) %>%
  apa.2way.table(iv1 = block_num, 
               iv2 = language,
               dv = correct_anticipation, 
               data = ., 
...
block_data %>%
  filter(age_group == "20 months" & trial_type == "post-switch" & n_blocks == 3) %>%
  apa.2way.table(iv1 = block_num, 
               iv2 = language,
               dv = correct_anticipation, 
               data = ., 
