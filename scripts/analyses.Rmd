...
trial_data <- data_eyetracking_clean %>%
  ...
  mutate(correct = case_when(looking_ms_target > looking_ms_distractor ~ 1, 
         ...
         no_anticipation = case_when(looking_ms_target + looking_ms_distractor == 0 ~ 1, 
                              TRUE ~ 0),
         total_anticipation = 1 - no_anticipation) %>% #filter(no_anticipation == 0) %>% ### keep the circle looks!
  mutate(block_num = (as.numeric(trial_num)+2) %/% 3) %>%
  mutate(block_num = as.factor(block_num))
...
trial_data %>%
  ...
  ggplot(aes(x = trial_num, y = correct, color = language, shape = language, fill = language)) +
  ...
  scale_fill_manual("Language group", values = c("#D55E00", "#0072B2")) +
  ...
  ggtitle("Correct anticipation by trial for Study 1a (age 7 months)")


ggsave(paste0("plot_by_trial_", dataset, ".png"),
       path = here("output/table_figure/"),
...
trial_data %>%
  ...
  ylab("Proportion of infants with \ncorrect anticipation") +
  labs(color  = "Language group", linetype = "Language group", shape = "Language group") +
  ggtitle("Correct anticipation by trial for Study 2 (age 20  months)")
